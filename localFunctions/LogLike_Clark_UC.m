function [negLL, struct_out] = LogLike_Clark_UC(starting_values, data_input, other_inputs, KFS_on)
% ------------------------------------------------------------------------------------------------------
% MODEL IS:
% ------------------------------------------------------------------------------------------------------
% 					y(t) = trend(t) + cycle(t)
...
% 				  g(t) = g(t-1) + v(t)
%  a(L)*cycle(t) = u(t)
% where y*(t) = trend(t) = y~(t) = cycle, and shocks w(t) and u(t) are correlated
% ------------------------------------------------------------------------------------------------------
% STATE SPACE MODEL:
...
% CORRELATIONS PARAMETER ESTIMATED HERE
rho_ystar_ytild	= starting_values(6); 

% OTHER INPUTS
% prior on states (here I don't even use the grid input)
a00_in	= other_inputs.a00; 
P00_in	= other_inputs.P00;
% OTHER CORRELATION PARAMETER FIXED AT SOME OTHER VALUE, COULD BE ZERO, OR IN GRID SEARCH
rho_ystar_g			= 0;
% rho_ystar_ytild = 0;
rho_ytild_g			= 0;
